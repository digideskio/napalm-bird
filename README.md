[![PyPI](https://img.shields.io/pypi/v/napalm-bird.svg)](https://pypi.python.org/pypi/napalm-bird)
[![PyPI](https://img.shields.io/pypi/dm/napalm-bird.svg)](https://pypi.python.org/pypi/napalm-bird)
[![Build Status](https://travis-ci.org/napalm-automation/napalm-bird.svg?branch=master)](https://travis-ci.org/napalm-automation/napalm-bird)
...
call it `BIRD` so everytime you see `BIRD` on this repo, replace it with the name you just
picked.
...
1. Replace the occurrences on this file of `BIRD`. Replace only the ones above
...
All the code should be inside `napalm_bird`. You are free to organize your code as you want,
however, there are some parts that have to be done in a certain way:

* `napalm_bird/__init__.py` - That file should import your class driver. That's what the
dynamic importer will expect.
* `napalm_bird/bird.py` - Here goes your driver.
* `napalm_bird/templates/` - We use this folder to store templates used by the `load_template`
method.
* `napalm_bird/utils/` - For consistency with other repos we recommend putting your additional
code here. Helper functions or anything you want to keep out of the main driver file.
* `napalm_bird/utils/textfsm_templates` - For consistency as well, we recommend keeping your
...
* `test/unit/bird/` - Here goes some configuration files that are used by `TestConfigDriver`.
* `test/unit/bird/mock_data/` - Here goes files that contain mocked data used by
                                    `TestGetterDriver`.
